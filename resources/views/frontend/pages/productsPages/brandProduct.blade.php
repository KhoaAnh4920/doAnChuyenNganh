...
<div class="features_items">
    <!--features_items-->
    @foreach($name_brand as $key => $name)
    <h2 class="title text-center">Sản phẩm của hãng {{$name->tenThuongHieu}}</h2>
    @endforeach

    @if(count($product_of_brand) > 0)
        @foreach($product_of_brand as $key => $pro)
        <div class="col-sm-4">
